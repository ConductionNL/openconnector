...
/**
 * AuthenticationExtensionTest
 *
 * Comprehensive unit tests for the AuthenticationExtension class to verify
...
/**
 * Authentication Extension Test Suite
 *
 * Comprehensive unit tests for Twig authentication extension including
 * function registration and configuration.
 *
 * @coversDefaultClass AuthenticationExtension
 */
class AuthenticationExtensionTest extends TestCase
{
    ...
    public function testGetFunctions(): void
    {
        ...
        $this->assertContains('oauthToken', $functionNames);
        $this->assertContains('decosToken', $functionNames);
        $this->assertContains('jwtToken', $functionNames);
    }

    /**
     * Test oauthToken function registration
     *
     * @covers ::getFunctions
     * @return void
     */
    public function testOauthTokenFunctionRegistration(): void
    {
        $functions = $this->extension->getFunctions();
        
        $oauthTokenFunction = null;
        foreach ($functions as $function) {
            if ($function->getName() === 'oauthToken') {
                $oauthTokenFunction = $function;
                break;
            }
        }
        
        $this->assertNotNull($oauthTokenFunction);
        $this->assertEquals('oauthToken', $oauthTokenFunction->getName());
        $this->assertEquals([\OCA\OpenConnector\Twig\AuthenticationRuntime::class, 'oauthToken'], $oauthTokenFunction->getCallable());
    }

    /**
     * Test decosToken function registration
     *
     * @covers ::getFunctions
     * @return void
     */
    public function testDecosTokenFunctionRegistration(): void
    {
        $functions = $this->extension->getFunctions();
        
        $decosTokenFunction = null;
        foreach ($functions as $function) {
            if ($function->getName() === 'decosToken') {
                $decosTokenFunction = $function;
                break;
            }
        }
        
        $this->assertNotNull($decosTokenFunction);
        $this->assertEquals('decosToken', $decosTokenFunction->getName());
        $this->assertEquals([\OCA\OpenConnector\Twig\AuthenticationRuntime::class, 'decosToken'], $decosTokenFunction->getCallable());
    }

    /**
     * Test jwtToken function registration
     *
     * @covers ::getFunctions
     * @return void
     */
    public function testJwtTokenFunctionRegistration(): void
    {
        $functions = $this->extension->getFunctions();
        
        $jwtTokenFunction = null;
        foreach ($functions as $function) {
            if ($function->getName() === 'jwtToken') {
                $jwtTokenFunction = $function;
                break;
            }
        }
        
        $this->assertNotNull($jwtTokenFunction);
        $this->assertEquals('jwtToken', $jwtTokenFunction->getName());
        $this->assertEquals([\OCA\OpenConnector\Twig\AuthenticationRuntime::class, 'jwtToken'], $jwtTokenFunction->getCallable());
    }
    ...
    public function testFunctionCallableValidity(): void
    {
        ...
        foreach ($functions as $function) {
            ...
            $this->assertEquals(\OCA\OpenConnector\Twig\AuthenticationRuntime::class, $callable[0]);
    ...
    public function testMultipleCallsToGetFunctions(): void
    {
        ...
        $this->assertCount(3, $functions1);
        $this->assertCount(3, $functions2);
    }
