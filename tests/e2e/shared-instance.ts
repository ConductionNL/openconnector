 ...
 *   INTEGRIQ_E2E_ALLOW_SHARED_INSTANCE  this app only
 *
...
export const APP_ID = 'integriq'

/** The app-specific opt-in variable. Holds an origin, not a boolean. */
export const SHARED_INSTANCE_FLAG = 'INTEGRIQ_E2E_ALLOW_SHARED_INSTANCE'
 ...
 *
 * Resolution order:
 *  1. `INTEGRIQ_E2E_OCC`, a complete prefix, for any rig shape the guesses
 *     below do not cover.
 *  2. `INTEGRIQ_E2E_CONTAINER` or `NEXTCLOUD_CONTAINER`, a container name,
...
export function occPrefix(env: NodeJS.ProcessEnv = process.env): string[] {
	const explicit = (env.INTEGRIQ_E2E_OCC ?? '').trim()
	if (explicit !== '') return explicit.split(/\s+/).filter((p) => p !== '')

	const container = (
		env.INTEGRIQ_E2E_CONTAINER ?? env.NEXTCLOUD_CONTAINER ?? ''
	).trim()
	if (container !== '') {
